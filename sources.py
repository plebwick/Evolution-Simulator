...
class Source:
    ...
    def respawn(sim):
        if sim.season == "Spring": season_frequency = 4
        if sim.season == "Summer": season_frequency = 3
        if sim.season == "Autumn": season_frequency = 4
        if sim.season == "Winter": season_frequency = 5
