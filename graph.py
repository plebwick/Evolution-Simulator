import pygame

class Graph:
    def __init__(self,
                 gene,
                 colour,
                 display,
                 values = []):
        self.gene = gene
        self.colour = colour
        self.display = display
        self.values = values

    def log(self, sim):
        people_length = len(sim.people)
        total = sum(getattr(person.genes, self.gene)for person in sim.people)
        if people_length > 0: average = total/people_length
        self.values.append(average)

    def draw(self,sim):
        graph_x_size = round(sim.screen_x*0.8,-2)
        graph_y_size = round(sim.screen_y*0.8,-2)

        x_offset = (sim.screen_x - sim.screen_x*0.8)/2
        y_offset = (sim.screen_y - sim.screen_y*0.8)/2

        values = self.values[-round(sim.graph_time):]

        min_value = min(values)-min(values)*0.1 if values else 0
        max_value = max(values)+max(values)*0.1 if values else 1

        step = len(values)/(graph_x_size)

        difference = (max_value-min_value)

        if len(values) > graph_x_size:
            points = [(i+x_offset, (graph_y_size + y_offset - graph_y_size*(values[int(i*step)]-min_value)/difference))
                    for i in range(0, round(graph_x_size))]
        else:
            points = [(i+x_offset, (graph_y_size + y_offset - graph_y_size*(values[int(i)]-min_value)/difference))
                    for i in range(0, len(values))]

        if len(points) > 2: pygame.draw.lines(sim.screen, self.colour, 0, points, 4)

        display_time = min(len(values), sim.graph_time)

        #draws the text
        sim.draw_text(x_offset, y_offset+graph_y_size+10, display_time, "ticks ago")
        sim.draw_text(x_offset+graph_x_size, y_offset+graph_y_size+10, 0, "ticks ago")

        sim.draw_text(x_offset-50, y_offset, round(max_value,2), "")
        sim.draw_text(x_offset-50, y_offset+graph_y_size, round(min_value,2), "")